...
async function getM3u8Url(websiteUrl, buttonId, buttonClassName, enableBypass) {
	let browser;
	try {
		...
		if (enableBypass) {
			...
			await page.evaluateOnNewDocument(() => {
				// Disable breakpoints by overriding the debugger statement
			...
			page.on("request", (request) => {
				...
				// List of known third-party scripts to ignore
				const ignoredScripts = [
					"googletagmanager.com",
					"google-analytics.com",
					"adsbygoogle.js",
					// Add more known third-party URLs as needed
				];

				if (ignoredScripts.some((script) => url.includes(script))) {
					console.log(`Ignoring script: ${url}`);
					request.abort();
				} else {
					request.continue();
				}
		...
		// Use button_id if provided, otherwise fallback to button_class_name
		let playButton;
		if (buttonId) {
			playButton = await page.$(`#${buttonId}`);
		} else {
			playButton = await page.$(`.${buttonClassName}`);
		}
...
(async () => {
	...
	const buttonId = settings.button_id;
	const buttonClassName = settings.button_class_name;
	const enableBypass = settings.enable_bypass;
	...
	const manifestUrl = await getM3u8Url(
		websiteUrl,
		buttonId,
		buttonClassName,
		enableBypass,
